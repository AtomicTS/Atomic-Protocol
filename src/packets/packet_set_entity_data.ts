...
export type MetadataDictionary = {
  ...
  type:
    ...
    | "vec3f";
  value:
    | { key: "flags"; value: MetadataFlags1 }
    | { key: "flags_extended"; value: MetadataFlags2 }
    | { key: "seat_third_person_camera_radius"; value: number }
    | { key: "seat_camera_relax_distance_smoothing"; value: number };
}[];

export interface EntityProperties {
  ints: { index: number; value: number }[];
  floats: { index: number; value: number }[];
}
...
export const SetEntityDataPacketInfo: import("./metadata").PacketMetadata = {
  id: undefined,
